...
*/
	...
	public $content_path 	= 'backend/news';			//set view partial page 
	public $redirect 		= 'NewsCrud';				//set redirect after set flash data
	public $name			= 'News';					//set insert/update message
...
*/
	...
	public function create(){
		...
		if($this->form_validation->run() == TRUE && $this->upload->do_upload('news_img') == TRUE):
			$data = $this->input->post();
			unset($data['submit']);

			$img_data = $this->upload->data();	
			$data['news_image_path'] = $img_data['raw_name'].$img_data['file_ext'];
	...
	public function update($id,$img_name){
		...
		if($this->form_validation->run() == TRUE):
			if(!isset($_FILES['news_img']) || $_FILES['news_img']['error'] == UPLOAD_ERR_NO_FILE ):
				...
				if($this->upload->do_upload('news_img') == TRUE):
					$data = $this->input->post();
					unset($data['submit']);

					$img_data = $this->upload->data();	
					$data['news_image_path'] = $img_data['raw_name'].$img_data['file_ext'];
					unlink('uploads/news/'.$img_name);
					return $this->set_flash_redir($this->model->update($id,$data),
					 							$this->name." Updated Successfully",
					 							"Failed to Update!");
				else:
