...
*/
	...
	public $content_path 	= 'backend/award';			//set view partial page 
	public $redirect 		= 'Award';					//set redirect after set flash data
	public $name			= 'Award';					//set insert/update message
	public $modelname		= 'Awardmodel';				//set model name
...
*/
	...
	public function create(){	
		...
		$this->form_validation->set_rules('award_title', 'Title', 'required');

		if($this->form_validation->run() == TRUE && $this->upload->do_upload('award_image') == TRUE):
			$data = $this->input->post();
			unset($data['submit']);

			$img_data = $this->upload->data();	
			$data['award_image'] = $img_data['raw_name'].$img_data['file_ext'];
	...
	public function update($id,$img_name){
		...
		if($this->form_validation->run() == TRUE):
			if(!isset($_FILES['award_image']) || $_FILES['award_image']['error'] == UPLOAD_ERR_NO_FILE ):
				...
				if($this->upload->do_upload('award_image') == TRUE):
					$data = $this->input->post();
					unset($data['submit']);

					$img_data = $this->upload->data();	
					$data['award_image'] = $img_data['raw_name'].$img_data['file_ext'];
					unlink('uploads/award/'.$img_name);
					return $this->set_flash_redir($this->model->update($id,$data),
					 							$this->name." Updated Successfully",
					 							"Failed to Update!");
				else:
